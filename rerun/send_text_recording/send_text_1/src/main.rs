...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let mut session = Session::init("record_text1", true);
    ...
    let one_millis = std::time::Duration::from_millis(100);
    loop {
        let text = rerun::components::TextEntry::from_body(format!("Ten Counter is {}", counter));
        MsgSender::new("ten_ms")
            .with_splat(text)?
            .send(&mut session)?;
